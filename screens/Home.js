...
const Home = () => {
  ...
  const handleAddMealPress = () => {
    console.log('Botão Adicionar Refeição pressionado');
  };

  const [waterHistory, setWaterHistory] = useState([]);

  const addToWaterHistory = (amount) => {
    const newHistory = [...waterHistory, { id: waterHistory.length + 1, amount }];
  ...
  const calculateTotalWater = () => {
    const totalWater = waterHistory.reduce((total, item) => total + item.amount, 0);
    return totalWater;
  };

  const data = [
    { id: 1, image: require('..//assets/images/cafedamanha.png') },
    { id: 2, image: require('..//assets/images/almoco.png') },
    { id: 3, image: require('..//assets/images/lanche.png') },
    { id: 4, image: require('..//assets/images/jantar.png') },
  ];
    ...
    <View style={[styles.container, { backgroundColor: '#FFEBA3' }]}>
      ...
      <View style={styles.waterHistoryBox}>
          ...
          renderItem={({ item }) => (
            <View style={styles.waterHistoryItem}>
              ...
              <TouchableOpacity onPress={() => removeWaterItem(item.id)}>
                <Ionicons name="trash-bin" size={18} color="#000" style={styles.trashIcon} />
              </TouchableOpacity>
            </View>
          )}
          horizontal
        ...
        <View style={styles.totalWaterContainer}>
          <Text style={styles.totalWaterText}>Total de Água: {calculateTotalWater()} ml</Text>
        </View>
      </View>

      <View style={styles.buttonContainer}>
        <TouchableOpacity style={[styles.button, { marginTop: 6 }]} onPress={() => addToWaterHistory(1000)}>
        ...
        <TouchableOpacity style={[styles.button, { marginTop: 6 }]} onPress={() => addToWaterHistory(500)}>
          <View style={styles.buttonContent}>
        ...
        <TouchableOpacity style={[styles.button, { marginTop: 6 }]} onPress={() => addToWaterHistory(200)}>
          <View style={styles.buttonContent}>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('MacarronadaFit')}></TouchableOpacity>
    </View>
  );
};
